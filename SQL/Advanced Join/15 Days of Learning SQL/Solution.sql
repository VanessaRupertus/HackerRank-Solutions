...
is the id of the submission, hacker_id is the id of the hacker who made the submission,
...
SELECT
    ...
    (SELECT COUNT(DISTINCT HACKER_ID)
     FROM SUBMISSIONS S
     WHERE S.SUBMISSION_DATE = S2.SUBMISSION_DATE AND
             (SELECT COUNT(DISTINCT S3.SUBMISSION_DATE)
              FROM SUBMISSIONS S3
              WHERE S3.HACKER_ID = S.HACKER_ID AND
                      S3.SUBMISSION_DATE < S2.SUBMISSION_DATE) = DATEDIFF(S2.SUBMISSION_DATE, '2016-03-01')),
    (SELECT HACKER_ID
     FROM SUBMISSIONS S
...
FROM
    (SELECT DISTINCT SUBMISSION_DATE FROM SUBMISSIONS) S2
